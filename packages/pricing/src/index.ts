export type AddonId = "scale" | "remove_ads" | "custom_domain" | "whatsapp";
...
/**
 * Catalog of paid add-ons, shared between the app (checkout/enforcement)
 * and the marketing site (pricing page) — a single source of truth so the
 * public price list can never drift from what's actually charged.
 *
 * Everything here is a one-off purchase per event; there is no
 * subscription/recurring billing in this phase. Prices in BRL cents. The
 * anti-penetra feature is deliberately absent from this list: it is
 * included free on every event, paid add-ons or not.
 */
...
export const FLAT_ADDON_PRICES: Record<Exclude<AddonId, "scale">, { label: string; amountCents: number }> = {
  ...
  whatsapp: { label: "Convite por WhatsApp", amountCents: 990 },
};
